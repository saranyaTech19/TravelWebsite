...
interface IndiaLocalToursPageProps {
  onExplore: (tour: any) => void;
  onBookClick: () => void;
}

const IndiaLocalToursPage: React.FC<IndiaLocalToursPageProps> = ({ onExplore, onBookClick }) => {
  const [northTours, setNorthTours] = useState<any[]>([]);
  const [southTours, setSouthTours] = useState<any[]>([]);
  ...
  useEffect(() => {
    const fetchTours = async () => {
      ...
      try {
        ...
        const indiaTours = data.filter((t: any) => t.region === 'India');

        setNorthTours(indiaTours.filter((tour: any) => {
        ...
        setSouthTours(indiaTours.filter((tour: any) => {
          const cat = tour.category?.toLowerCase() || '';
          const title = tour.title?.toLowerCase() || '';
          const loc = tour.location?.toLowerCase() || '';
        ...
        console.error('Error fetching India local tours:', err);
      } finally {
        setIsLoading(false);
      }
    };
        ...
        <div className="flex items-end justify-between mt-auto">
          <div className="flex flex-col">
            ...
            <span className="text-lg font-bold text-[#1B6B93]">
              {tour.price ? (tour.price.toString().startsWith('$') || tour.price.toString().startsWith('₹') || tour.price.toString().startsWith('AED') ? tour.price : `$${tour.price}`) : '$0'}
            </span>
          </div>
          <button
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="w-8 h-8 border-4 border-[#00A9D7] border-t-transparent rounded-full animate-spin"></div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-brand-bg">
      {/* North India Tours Section Header/Banner */}
      <div
        className="w-full h-[360px] mx-auto relative flex flex-col items-center justify-start lg:pt-24 pb-20 overflow-visible   p-cards"
        style={{
      ...
      >
        <div className="lg:text-white text-black lg:text-[38px] text-2xl font-bold  lg:mb-12 mb-4">
          North India Tours
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 relative w-full px-6 md:px-14">
          {northTours.length === 0 ? (
            <div className="col-span-4 py-20 text-center font-bold text-slate-400">No tours found.</div>
          ) : (
            northTours.slice(0, 4).map(renderCard)
          )}
        </div>
      </div>

      {/* Remaining North India Tours */}
      {northTours.length > 4 && (
        <div className="max-w-[1440px] mx-auto lg:pb-16 pb-8 lg:pt-[200px] pt-[22px] px-6 md:px-14 lg:mt-[40px]">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {northTours.slice(4).map(renderCard)}
          </div>
        </div>
      )}
      ...
      {southTours.length > 0 && (
        <div className={`max-w-[1440px] mx-auto lg:py-8 px-6 md:px-14 ${northTours.length <= 4 ? 'lg:pt-[200px] pt-[22px]' : ''}`}>
...
export default IndiaLocalToursPage;
