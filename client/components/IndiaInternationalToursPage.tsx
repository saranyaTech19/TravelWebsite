...
interface IndiaInternationalToursPageProps {
  onExplore: (tour: any) => void;
  onBookClick: () => void;
}

const IndiaInternationalToursPage: React.FC<IndiaInternationalToursPageProps> = ({ onExplore, onBookClick }) => {
  ...
  useEffect(() => {
    const fetchTours = async () => {
      ...
      try {
        ...
        setTours(indiaTours.filter((tour: any) => {
          const cat = tour.category?.toLowerCase() || '';
          return cat === 'standard' || cat === 'international trips from india';
        }));
      } catch (err) {
        console.error('Error fetching international tours from India:', err);
      } finally {
        setIsLoading(false);
      }
    };
    ...
    <div className="min-h-screen bg-brand-bg">
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 relative w-full px-6 md:px-14">
          ...
          ) : tours.slice(0, 4).map(renderCard)}
        </div>
      </div>
...
export default IndiaInternationalToursPage;
