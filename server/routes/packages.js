  ...
  try {
    ...
    const [result] = await db.query(
      `INSERT INTO tour_packages
        (title, slug, location, region, category, image_url, rating, duration, guest_capacity, tag, price, is_featured, overview, highlights, inclusions, exclusions, gallery, itinerary, flyer_url)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
  ...
  try {
    ...
    await db.query(
      `UPDATE tour_packages SET
        ...
        highlights=?, inclusions=?, exclusions=?, gallery=?, itinerary=?, flyer_url=?, updated_at=NOW()
       WHERE id=?`,
      [
        pkg.title, pkg.slug, pkg.location, pkg.region, pkg.category,
        pkg.image_url, pkg.rating, pkg.duration, pkg.guest_capacity,
